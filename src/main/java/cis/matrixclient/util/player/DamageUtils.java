...
import cis.matrixclient.interfaces.IExplosion;
import cis.matrixclient.interfaces.IRaycastContext;
...
public class DamageUtils {
    ...
    public static void init() {
        MatrixClient.EVENT_BUS.register(new DamageUtils());
    }
    ...
    public static double crystalDamage(PlayerEntity player, Vec3d crystal, BlockPos obsidianPos, boolean ignoreTerrain) {
        ...
        if (EntityUtils.getGameMode(player) == GameMode.CREATIVE && !(player == ModuleManager.getModule(FakePlayer.class).fakePlayer)) return 0;
